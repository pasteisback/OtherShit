...
Title.TextSize = 18
Title.Text = "Infinite Yield Modified v" .. currentVersion
...
Credits.Text = "Edge(original owner) // no.luv(new owner)"
...
CMDs[#CMDs + 1] = {NAME = 'executescript / escript', DESC = 'Executes a script.'}
CMDs[#CMDs + 1] = {NAME = 'unc', DESC = 'Executes UNC Test'}
CMDs[#CMDs + 1] = {NAME = 'sunc', DESC = 'Executes SUNC Test'}
...
addcmd('executescript', {'escript'}, function(args, speaker)
	local success, err = pcall(function()
		loadstring(args[1])()
	end)

	if success then
		notify('Scripter', 'Executed.')
	else
		notify('Scripter', 'Error: ' .. tostring(err))
	end
end)
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet("https://raw.githubusercontent.com/pasteisback/OtherShit/refs/heads/main/fuckbru")
		return HttpService:JSONDecode(versionJson)
	end)

	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify("Outdated", "uh js cry abt it.")
		end
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, Infinite Yield Modded may have issues or features that are not functioning correctly.") end
end)
